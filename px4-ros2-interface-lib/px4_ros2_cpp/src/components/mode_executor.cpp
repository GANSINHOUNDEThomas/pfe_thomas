...
#define VEHICLE_MODE_FLAG_STABILIZE_ENABLED    16   // 0b00010000 - Stabilization enabled. System stabilizes attitude and position but requires further control inputs to move.
#define VEHICLE_MODE_FLAG_HIL_ENABLED          32   // 0b00100000 - Hardware-in-the-loop (HIL) simulation mode. Motors/actuators are blocked but internal software is fully operational.
...
bool ModeExecutorBase::doRegister()
{
  ...
  settings.activate_mode_immediately =
    (_settings.activation == Settings::Activation::ActivateImmediately) || (_settings.activation == Settings::Activation::ActivateAlways);
...
void ModeExecutorBase::vehicleStatusUpdated(const px4_msgs::msg::VehicleStatus::UniquePtr & msg)
{
  ...
  _curr_nav_state = current_mode;// ajouté par moi
  _is_armed = msg->arming_state == px4_msgs::msg::VehicleStatus::ARMING_STATE_ARMED;
  const bool wants_to_activate_immediately =
    (_settings.activation == Settings::Activation::ActivateImmediately) && _was_never_activated; //ajouté par moi
